# gittish-core-protocol-organization-delete

This package defines the service interface that is responsible for deleting an
...
The standard course of action for this service does not include the deletion of

* owned repositories
* other data that is supplemental to the organization

In order to accomplish the complete deletion of an organization, one must use
the organization deletion workflow service, which in turn will call upon this
service.
...
```
cd <WORKSPACE_ROOT>/gittish-core-protocol-organization-delete
./configure
```
